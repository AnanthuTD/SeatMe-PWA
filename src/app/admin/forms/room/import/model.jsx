...
const RoomsModel = ({ data, setData }) => {
	...
	const handleOk = async () => {
		...
		try {
			const result = await axios.patch("/api/admin/rooms", data);
	...
	const updateSingle = async (record, index) => {
		...
		try {
			const result = await axios.patch("/api/admin/rooms", [record]);
			if (result.data) {
			...
			>
				{data.map((record, index) => (
					<Card key={index} className="my-4">
						<Row gutter={[16, 16]}>
							<Col span={12}>
								<p className="font-bold mb-1">Room Info:</p>

								<p>Room ID: {record.id}</p>
								<p>Columns: {record.cols}</p>
								<p>Rows: {record.rows}</p>
								<p>
									Is Available:{" "}
									{record.isAvailable ? "Yes" : "No"}
								</p>
								<p>Floor: {record.floor}</p>
								<p>Block: {record.blockId}</p>
							</Col>
