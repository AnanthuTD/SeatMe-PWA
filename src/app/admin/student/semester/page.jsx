...
const PromoteDemotePassout = () => {
    ...
    const handlePromote = async () => {
        try {
            ...
            await axios.patch('/api/admin/student/promote', { semester, year, program });
    ...
    const handleDemote = async () => {
        try {
            ...
            await axios.patch('/api/admin/student/demote', { semester, year, program });
    ...
    const handlePassout = async () => {
        try {
            ...
            await axios.patch('/api/admin/student/passout', { semester, year, program });
    ...
    return (
        <Form layout="vertical">
            ...
            <DepProSemCouSelect courseField={false} value={({program, semester})=>{setSemester(semester);setProgram(program)}} />
            ...
            <Form.Item label="Year of Admission (Optional)">
                <Input value={year} onChange={(e) => handleYearChange(e.target.value)} />
            </Form.Item>

            {/* Buttons for promote, demote, and pass out */}
            <Form.Item>
                <Button type="primary" onClick={handlePromote} disabled={isButtonDisabled} className='mr-2'>
                    Promote by 1
                </Button>
                <Button type="primary" onClick={handleDemote} disabled={isButtonDisabled} className='mr-2'>
                    Demote by 1
                </Button>
                <Button type="primary" onClick={handlePassout} disabled={isButtonDisabled}>
                    Pass Out
                </Button>
            </Form.Item>
        </Form>
